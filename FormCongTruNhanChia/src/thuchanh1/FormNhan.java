...
//            txtSo3.setText(so1 +" x " + so2 +" = "+ (so1 * so2));
//        }catch(Exception e){
...
try {
  ...
  try {
    ...
    int nhan = so1 * so2;
    txtSo3.setText(so1 + " x " + so2 + " = " + nhan);
  } catch (NumberFormatException e) {
    double dSo1, dSo2;
    try {
      dSo1 = Double.parseDouble(strSo1);
      dSo2 = Double.parseDouble(strSo2);
      double nhan = dSo1 * dSo2;
      txtSo3.setText(strSo1 + " x " + strSo2 + " = " + String.format("%.1f", nhan));
